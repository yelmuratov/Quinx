document.getElementById('copyClipboard')?.addEventListener('click', function() {
  ...
  const walletIdText = document.getElementById('walletID').textContent;
  navigator.clipboard.writeText(walletIdText).then(() => {
      // Change the icon to a check mark
      const icon = document.getElementById('copyClipboard');
      icon.classList.remove('fa-clipboard');
      icon.classList.add('fa-check');

      // Switch back to the clipboard icon after 1 second
      setTimeout(() => {
...
App = {
  ...
  loadAccount: async () => {
    ...
    web3.eth
      ...
      .then((accounts) => {
        ...
        document.getElementById("view_link").href = `https://explorer.apothem.network/address/${App.account}#transactions`
  ...
  tokenDetails: async () => {
    ...
    const availableToken = await App.contracts.token.methods
      ...
      if(userRole == 'individual'){
        ...
        parseInt(localStorage.getItem('userTokens')) + " QNX";
      ...
      document.querySelector("#tokenSymbol").innerHTML =
        ...
        document.querySelector("#tokenAvailable").innerHTML =
        web3.utils.fromWei(availableToken.toString(), "ether") + " QNX";
      document.querySelector("#tokenName").innerHTML =
        await App.contracts.token.methods.name().call();
      document.querySelector("#tokenSymbol").innerHTML =
        await App.contracts.token.methods.symbol().call();
  ...
  detailsToListTokenForSell: async () => {
    ...
    document.querySelector("#tokenBalance").innerHTML =
      web3.utils.fromWei(tokenBalance.toString(), "ether") + " QNX";
    document.querySelector("#tokenPrice").innerHTML =
      tokenPrice.toString() + " Wei";
  },
